...
const int relayPin = 33; // Define the pin for the relay switch
const int differenceThreshold = 950; // Set the difference threshold value
...
void setup() {
  ...
  if(!SD.begin()){
    ...
    writeFile(SD, "/data.txt", "Time,Load,RPM1,RPM2\n");
...
void loop() {
  ...
  // Calculate the difference betweent the rpms
  difference = abs(rpm - rpm2);
  ...
  String timestampSHORT = String(hours) + ":" + String(minutes) + ":" + String(seconds);
  
  lcd.setCursor(0,0);
  lcd.print("IP:172.28.211.86");
  lcd.setCursor(0,1);
  lcd.print("Time:");
  lcd.setCursor(5, 1);
  lcd.print(timestampSHORT);
  
  Weightstr = String(Weight);
  rpmstr = String(rpm);
  rpmstr2 = String(rpm2);

  // Combine timestamp other data for each row
  String entry = timestamp + "," + Weightstr + "," + rpmstr + "," + rpmstr2 + "\n";

  
  if ((millis() - timesincewrite) >= 1000){
...
void UpdateSlider() {
  ...
  // i avoid strings at all cost, hence all the code to start with "" in the buffer and build a
...
void SendXML() {
  ...
  // convert BitsA1 to string
  char bitsA1String[4];
  dtostrf(BitsA1, 4, 2, bitsA1String); // 4 is the total number of digits including decimal point and fraction digits
